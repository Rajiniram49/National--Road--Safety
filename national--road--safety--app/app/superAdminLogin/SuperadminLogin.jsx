...
export default function SuperadminLogin() {
  ...
  const moveHomePage = () => {
    ...
    signInWithEmailAndPassword(auth, email, password)
      .then(async (userCredential) => {
        ...
        console.log('Super Admin signed in:', user.email);
        await AsyncStorage.setItem('Adminuser', user.email);
        router.push('/Superadmin/superAdminDashboard');
      })
      .catch((error) => {
        console.error('Login Error:', error.message);
        setError('Are you sure! you are an Super Admin?, Use the correct credentials');
        Alert.alert('Are you sure! you are an Super Admin?, Use the correct credentials');
      });
  };
  ...
  const login = async () => {
    onAuthStateChanged(auth, (user) => {
      if (user) {
        console.log('Super Admin User is signed in');
        router.push('/Superadmin/superAdminDashboard');
      } else {
        console.log('Super Admin User is signed out');
      }
    });
    return () => unsubscribe();
  }
